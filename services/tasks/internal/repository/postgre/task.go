...
const (
	baseTaskGetQuery     = `SELECT uuid, name, text, status, created_by, started_at, ended_at, created_at, updated_at FROM tasks`
	createTaskQuery      = `INSERT INTO tasks (name, text, status, created_by, started_at, ended_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING uuid`
	updateTaskQuery      = `UPDATE tasks SET name = $2, text = $3, status = $4, started_at = $5, ended_at = $6, updated_at = $7 WHERE uuid = $1`
	patchStatusTaskQuery = `UPDATE tasks SET status = $2, updated_at = $3 WHERE uuid = $1`
)

var _ adapters.TaskRepository = (*taskRepository)(nil)
...
func (r *taskRepository) Update(ctx context.Context, task domain.TaskUpdate) error {
	tr := global.Tracer(adapters.ServiceTask)
	_, span := tr.Start(ctx, spanDefaultTask+".Update")
	defer span.End()
...
func (r *taskRepository) PatchStatus(ctx context.Context, id uuid.UUID, status string) error {
	tr := global.Tracer(adapters.ServiceTask)
	_, span := tr.Start(ctx, spanDefaultTask+".PatchStatus")
	defer span.End()
